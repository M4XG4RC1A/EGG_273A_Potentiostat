...
class MainPage(ctk.CTkFrame):
    ...
    #     # ---- Method metadata ----
    #     meta_frame = ctk.CTkFrame(f)
    #     meta_frame.pack(fill="x", padx=12, pady=8)

    #     ctk.CTkLabel(meta_frame, text="Method Name:").grid(row=0, column=0, sticky="w")
    #     self.method_name_entry = ctk.CTkEntry(meta_frame)
    #     self.method_name_entry.grid(row=0, column=1, padx=4, pady=2)

    #     ctk.CTkLabel(meta_frame, text="Description:").grid(row=1, column=0, sticky="w")
    #     self.method_desc_entry = ctk.CTkEntry(meta_frame)
    #     self.method_desc_entry.grid(row=1, column=1, padx=4, pady=2)

    #     ctk.CTkLabel(meta_frame, text="Category:").grid(row=2, column=0, sticky="w")
    #     self.method_category_entry = ctk.CTkEntry(meta_frame)
    #     self.method_category_entry.insert(0, "Custom")
    #     self.method_category_entry.grid(row=2, column=1, padx=4, pady=2)

    #     # ---- Scrollable area for Inputs + Process ----
    #     scroll_frame = ctk.CTkScrollableFrame(f, width=700, height=400)
    #     scroll_frame.pack(fill="both", expand=True, padx=12, pady=8)

    #     # ---- Inputs section ----
    #     inputs_label = ctk.CTkLabel(scroll_frame, text="Inputs:", font=ctk.CTkFont(size=14, weight="bold"))
    #     inputs_label.pack(anchor="w", pady=(0,4))

    #     self.input_blocks = []
    #     inputs_frame = ctk.CTkFrame(scroll_frame)
    ...
    #         name_entry = ctk.CTkEntry(block_frame, placeholder_text="Label")
    #         name_entry.grid(row=0, column=0, padx=4)
    #         var_entry = ctk.CTkEntry(block_frame, placeholder_text="Variable name")
    #         var_entry.grid(row=0, column=1, padx=4)
    #         default_entry = ctk.CTkEntry(block_frame, placeholder_text="Default value")
    #         default_entry.grid(row=0, column=2, padx=4)
    #         type_option = ctk.CTkOptionMenu(block_frame, values=["float", "int"])
    #         type_option.grid(row=0, column=3, padx=4)
    ...
    #     add_input_btn = ctk.CTkButton(scroll_frame, text="+ Add Input", command=add_input_block)
    #     add_input_btn.pack(anchor="w", pady=4)

    #     # ---- Process section ----
    #     process_label = ctk.CTkLabel(scroll_frame, text="Process:", font=ctk.CTkFont(size=14, weight="bold"))
    #     process_label.pack(anchor="w", pady=(8,4))

    #     process_frame = ctk.CTkFrame(scroll_frame)
    #     process_frame.pack(fill="x", pady=2)

    #     self.process_blocks = []

    #     def add_repeat_block():
    #         repeat_frame = ctk.CTkFrame(process_frame)
    #         repeat_frame.pack(fill="x", pady=4, padx=8)

    #         repeat_entry = ctk.CTkEntry(repeat_frame, placeholder_text="Repeat variable (e.g. C)")
    #         repeat_entry.pack(side="left", padx=4)

    #         for_loops_frame = ctk.CTkFrame(repeat_frame)
    #         for_loops_frame.pack(fill="x", pady=4)

    #         def add_for_loop():
    #             loop_frame = ctk.CTkFrame(for_loops_frame)
    #             loop_frame.pack(fill="x", pady=2)

    #             start_entry = ctk.CTkEntry(loop_frame, placeholder_text="Start")
    #             start_entry.pack(side="left", padx=2)
    #             end_entry = ctk.CTkEntry(loop_frame, placeholder_text="End")
    #             end_entry.pack(side="left", padx=2)
    #             step_entry = ctk.CTkEntry(loop_frame, placeholder_text="Step")
    #             step_entry.pack(side="left", padx=2)

    #             # Commands inside the loop
    #             commands_frame = ctk.CTkFrame(loop_frame)
    #             commands_frame.pack(fill="x", pady=2)

    #             def add_command(cmd_type):
    #                 cmd_frame = ctk.CTkFrame(commands_frame)
    #                 cmd_frame.pack(fill="x", pady=1)
    #                 val_entry = ctk.CTkEntry(cmd_frame, placeholder_text="Value")
    #                 val_entry.pack(side="left", padx=2)
    #                 ctk.CTkLabel(cmd_frame, text=cmd_type).pack(side="left", padx=2)
    #                 return {"type": cmd_type, "entry": val_entry}

    #             mean_cmd = add_command("MEAN")
    #             delay_cmd = add_command("DELAY")
    #             output_cmd = add_command("OUTPUT")  # fixed OUTPUT(Vout=V,Iout=I)

    #             return {"start": start_entry, "end": end_entry, "step": step_entry,
    #                     "commands": [mean_cmd, delay_cmd, output_cmd]}

    #         add_loop_btn = ctk.CTkButton(for_loops_frame, text="+ Add For Loop", command=add_for_loop)
    #         add_loop_btn.pack(anchor="w", pady=2)

    #         self.process_blocks.append({"repeat_var": repeat_entry, "for_loops_frame": for_loops_frame})

    #     add_repeat_btn = ctk.CTkButton(process_frame, text="+ Add Repeat Block", command=add_repeat_block)
    #     add_repeat_btn.pack(anchor="w", pady=4)

    #     # ---- Save method button ----
    #     save_btn = ctk.CTkButton(scroll_frame, text="Save Method", command=self._save_new_method)
    ...
    def _build_new_method_tab(self):
        ...
        # Static header for inputs
        header_frame = ctk.CTkFrame(inputs_frame)
        header_frame.pack(fill="x", pady=(0,2))
        ctk.CTkLabel(header_frame, text="Label", width=100, anchor="w").pack(side="left", padx=2)
        ctk.CTkLabel(header_frame, text="Variable Name", width=120, anchor="w").pack(side="left", padx=2)
        ctk.CTkLabel(header_frame, text="Default Value", width=100, anchor="w").pack(side="left", padx=2)
        ctk.CTkLabel(header_frame, text="Type", width=80, anchor="w").pack(side="left", padx=2)

        def add_input_block():
            block_frame = ctk.CTkFrame(inputs_frame)
            block_frame.pack(fill="x", pady=2)

            name_entry = ctk.CTkEntry(block_frame, placeholder_text="Label", width=100)
            name_entry.pack(side="left", padx=2)
            var_entry = ctk.CTkEntry(block_frame, placeholder_text="Variable Name", width=120)
            var_entry.pack(side="left", padx=2)
            default_entry = ctk.CTkEntry(block_frame, placeholder_text="Default Value", width=100)
            default_entry.pack(side="left", padx=2)
            type_option = ctk.CTkOptionMenu(block_frame, values=["float", "int"], width=80)
            type_option.pack(side="left", padx=2)
        ...
        def add_repeat_block():
            ...
            def add_for_loop():
                ...
                def add_command(cmd_type):
                    cmd_frame = ctk.CTkFrame(commands_frame)
                    cmd_frame.pack(fill="x", pady=1)
                    val_entry = ctk.CTkEntry(cmd_frame, placeholder_text=cmd_type)
                    val_entry.pack(side="left", padx=2)
                    ctk.CTkLabel(cmd_frame, text=cmd_type).pack(side="left", padx=2)
                    return {"type": cmd_type, "entry": val_entry}
    ...
    def _save_new_method(self):
        # ---- 1. Validate method metadata ----
        name = self.method_name_entry.get().strip()
        desc = self.method_desc_entry.get().strip()
        category = self.method_category_entry.get().strip() or "Custom"
        ...
        for block in self.input_blocks:
            label = block["label"].get().strip()
            var = block["var"].get().strip()
            default = block["default"].get().strip()
            typ = block["type"].get()
            if label and var:
                try:
                    default_val = float(default) if typ == "float" else int(default)
                except ValueError:
                    default_val = 0.0 if typ == "float" else 0
                inputs.append({
                    "label": label,
                    "type": typ,
                    "default": default_val,
        ...
        # ---- 3. Static output ----
        output = [
            {"label": "Potential(mV)", "type": "float", "variable": "Vout"},
            {"label": "Current(A)", "type": "float", "variable": "Iout"}
        ]

        # ---- 4. Collect Process ----
        process_lines = []
        for repeat_block in self.process_blocks:
            repeat_var = repeat_block["repeat_var"].get().strip() or "C"
            process_lines.append(f"REPEAT({repeat_var}){{")

            for_loop_frames = repeat_block["for_loops_frame"].winfo_children()
            for loop_frame in for_loop_frames:
                entries = [w for w in loop_frame.winfo_children() if isinstance(w, ctk.CTkEntry)]
                if len(entries) < 3:
                    continue
                start, end, step = [e.get().strip() for e in entries[:3]]
                process_lines.append(f"    FOR_RANGEV({start},{end},{step}){{")

                # Commands inside loop
                commands_frame = loop_frame.winfo_children()[3]  # commands_frame
                cmd_lines = []
                for cmd_frame in commands_frame.winfo_children():
                    cmd_entry = cmd_frame.winfo_children()[0]  # Entry
                    cmd_type = cmd_frame.winfo_children()[1].cget("text").upper()
                    val = cmd_entry.get().strip()
                    if cmd_type == "OUTPUT":
                        cmd_lines.append("OUTPUT(Vout=V,Iout=I)")
                    else:
                        cmd_lines.append(f"{cmd_type}({val})")
                # join commands with commas
                process_lines.append("        " + ",\n        ".join(cmd_lines))
                process_lines.append("    };")  # end for loop
            process_lines.append("}")  # end repeat

        process_str = "\n".join(process_lines) if process_lines else "Hello"
        ...
        # ---- 6. Save JSON to file ----
        folder = os.path.join("Methods", "Custom")
        os.makedirs(folder, exist_ok=True)
        filename_base = os.path.join(folder, f"{name}.json")
        filename = filename_base
        counter = 1
        while os.path.exists(filename):
            filename = filename_base.replace(".json", f"_{counter:03d}.json")
            counter += 1

        with open(filename, "w") as f:
            json.dump(method_dict, f, indent=2)

        messagebox.showinfo("Saved", f"Method '{name}' saved to {filename}")

        # ---- 7. Reset page ----
        self.method_name_entry.delete(0, "end")
        self.method_desc_entry.delete(0, "end")
        self.method_category_entry.delete(0, "end")
        self.method_category_entry.insert(0, "Custom")

        for block in self.input_blocks:
            block["label"].destroy()
            block["var"].destroy()
            block["default"].destroy()
            block["type"].destroy()
        self.input_blocks.clear()

        for repeat_block in self.process_blocks:
            repeat_block["repeat_var"].destroy()
            repeat_block["for_loops_frame"].destroy()
        self.process_blocks.clear()
